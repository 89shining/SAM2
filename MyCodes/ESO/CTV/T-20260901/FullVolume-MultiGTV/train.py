...
from model import build_multigtv
...
def collate(v):
 ...
 return collate_fn(v,dict_key='fullvolume_multigtv_ctv')
...
def validate(core,cases,dev):
 ...
 for c in cases.values():
  b,p=c.video_with_context(list(range(len(c.image))));b=collate([b]).to(dev);q=predprob(c,midpoint_bidirectional(core,b,p));o=c.restore_probability_to_original(q)>.5;g=sitk.GetArrayFromImage(c.original_ctv)>0;r={'patient':c.patient,'dice':dice(o,g),'hd95_mm':hd95(o,g,c.spacing_zyx),'asd_mm':asd(o,g,c.spacing_zyx)};r.update(boundary_metrics(o,g,c.spacing_zyx[0]));rows.append(r)
 return rows
def materialize(core,c,plan,dev):
 b,p=c.video_with_context(plan['source_indices']);b=collate([b]).to(dev)
...
def main():
 ...
 a.out.mkdir(parents=True,exist_ok=True);sp=load_splits(SPLIT_PATH);f=sp['folds'][a.fold];lookup={p.name:p for p in patient_dirs(DATA_ROOT,'train')};tr={x:CTVGTVCase(lookup[x]) for x in f['train']};va={x:CTVGTVCase(lookup[x]) for x in f['val']};meta={'experiment':'Independent FullVolume-MultiCT-MultiGTV','fold':a.fold,'lambda_rank':a.lambda_rank,'split_sha256':sha(SPLIT_PATH),'train_ids':sorted(tr),'validation_ids':sorted(va),'same_control_as':'FullVolume-NoPrompt','trainable':'image Q/V LoRA; prompt encoder; memory encoder; memory attention; mask decoder; MultiCT fusion; MultiGTV fusion'};cfg=a.out/'training_config.json'
 if cfg.exists():
 ...
 dev=torch.device(a.device);core,names=build_multigtv(SAM2_ROOT,MODEL_CFG,CHECKPOINT,dev,LORA_RANK,LORA_ALPHA,LORA_DROPOUT);first=sample_epoch_plans(tr.values(),1,SEED,WINDOW)[0];materialize(core,tr[first['patient']],first,dev);opt=torch.optim.AdamW([p for p in core.parameters() if p.requires_grad],lr=LR,weight_decay=WEIGHT_DECAY);ids={id(p) for p in core.parameters() if p.requires_grad};oids={id(p) for g in opt.param_groups for p in g['params']};
 ...
 for e in range(start,a.epochs+1):
  ...
  for plan in sample_epoch_plans(tr.values(),e,SEED,WINDOW):
   b,p=tr[plan['patient']].video_with_context(plan['source_indices']);b=collate([b]).to(dev);loss,info=episode(core,b,p,plan['order'],plan,a.lambda_rank);opt.zero_grad(set_to_none=True);loss.backward();opt.step();tl.append(float(loss.detach()));sl.append(float(info['seg_loss']));rl.append(float(info['rank_loss']))
  core.eval();vl=[]
  with torch.no_grad():
   for plan in sample_epoch_plans(va.values(),0,SEED,WINDOW):
    b,p=va[plan['patient']].video_with_context(plan['source_indices']);b=collate([b]).to(dev);loss,_=episode(core,b,p,plan['order'],plan,a.lambda_rank);vl.append(float(loss))
