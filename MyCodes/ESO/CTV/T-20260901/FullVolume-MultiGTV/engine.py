...
def episode(core,batch,prompts,order,plan=None,lam=0.):
 base=chunk(core,batch.flat_img_batch);_,vision,pos,sizes=core._prepare_backbone_features(base);outputs={'cond_frame_outputs':{},'non_cond_frame_outputs':{}};losses=[];scores={};preds={};pairs=0
 for step,target in enumerate(order):
  frame=target+1;ids=batch.flat_obj_to_img_idx[frame];vp=[x[:,ids] for x in pos];vf=lambda ix:[x[:,batch.flat_obj_to_img_idx[ix]] for x in vision];current=core.multislice_image_fusion(vf(frame-1),vf(frame),vf(frame+1),sizes);mask=prompts[frame].unsqueeze(0).to(current[0].device);core._multigtv_context=(prompts[frame-1].unsqueeze(0).to(mask.device),mask,prompts[frame+1].unsqueeze(0).to(mask.device))
  ...
  (outputs['cond_frame_outputs'] if step==0 else outputs['non_cond_frame_outputs'])[frame]=out;losses.append(segloss(out['pred_masks'],batch.masks[frame].unsqueeze(1).to(out['pred_masks'].device)));preds[target]=out['pred_masks_high_res']
  if plan is not None:
   mass=torch.sigmoid(out['pred_masks']).mean((-2,-1)).clamp(1e-6,1-1e-6);scores[plan['source_indices'][target]]=torch.logit(mass).mean()
...
def midpoint_bidirectional(core,batch,prompts):
 n=int(batch.num_frames)-2;mid=(n-1)//2;out={};
