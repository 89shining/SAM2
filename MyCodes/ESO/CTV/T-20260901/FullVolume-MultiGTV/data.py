...
class CTVGTVCase:
 ...
 def video_with_context(self,targets):
  idx=[max(0,targets[0]-1)]+list(targets)+[min(len(self.image)-1,targets[-1]+1)];frames=[_frame(self.rgb[z],self.ctv[z],i) for i,z in enumerate(idx)];prompts=torch.stack([_prompt(self.gtv[z]) for z in idx]);return VideoDatapoint(frames=frames,video_id=int(self.patient[2:]),size=(IMAGE_SIZE,IMAGE_SIZE)),prompts
