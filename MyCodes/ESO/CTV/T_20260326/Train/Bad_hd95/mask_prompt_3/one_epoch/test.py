...
DEFAULT_FINETUNED_CKPT = None
DEFAULT_TRAIN_OUTPUT_ROOT = None
...
def infer_selector_middle(
    ...
    prompt_ids = sorted(set([int(lower_id), int(upper_id)]))
    ...
    pred_selector = _propagate_to_mask(state, selector_predictor, gt_zyx, obj_id)
    middle_id = select_middle_from_pred_and_gt_hd95(
        pred_zyx=pred_selector,
        gt_zyx=gt_zyx,
        lower_id=lower_id,
        upper_id=upper_id,
    )
    return pred_selector, int(middle_id)


@torch.no_grad()
def infer_main_one_shot_three_prompts(
    main_predictor,
    frame_dir: Path,
    gt_zyx: np.ndarray,
    lower_id: int,
    ...
    # one-shot inference: single propagate after all 3 prompts are provided
    pred_main = _propagate_to_mask(state, main_predictor, gt_zyx, obj_id)
    return pred_main
...
def resolve_ckpt(finetuned_ckpt: Path | None, train_output_root: Path, allow_fallback: bool = True) -> Path:
    ...
    if allow_fallback and finetuned_ckpt is not None and finetuned_ckpt.exists():
        return finetuned_ckpt

    if allow_fallback and DEFAULT_FINETUNED_CKPT is not None and DEFAULT_FINETUNED_CKPT.exists():
...
def main():
    parser = argparse.ArgumentParser(
        "Test SAM2 one-shot inference: selector picks middle, main model propagates once with upper/lower/middle"
    )
    ...
    parser.add_argument("--selector-train-output-root", type=Path, default=DEFAULT_SELECTOR_TRAIN_OUTPUT_ROOT, help="First-round training output root for selector checkpoint.")
    ...
    parser.add_argument("--excel-name", type=str, default="one_epoch_one_shot_bad95_results.xlsx")
    args = parser.parse_args()

    if args.test_root is None:
        raise ValueError("Missing --test-root. Please run via run_train.py or pass it explicitly.")
    ...
    if args.train_output_root is None:
        raise ValueError("Missing --train-output-root. Please run via run_train.py or pass it explicitly.")
    ...
    main_ckpt_path = resolve_ckpt(args.finetuned_ckpt, args.train_output_root, allow_fallback=True)
    selector_ckpt_path = resolve_ckpt(args.finetuned_ckpt, args.selector_train_output_root, allow_fallback=False)
    ...
    main_predictor = build_sam2_video_predictor(
        args.model_cfg,
        str(main_ckpt_path),
        device=device,
    )
    ...
    print(f"[INFO] Using main checkpoint: {main_ckpt_path}")
    ...
    for pdir in patient_dirs:
        ...
        try:
            ...
            pred_selector, middle_id = infer_selector_middle(
            ...
            pred_one_shot = infer_main_one_shot_three_prompts(
            ...
            dice_selector = dice_3d(pred_selector, gt_zyx)
            dice_one_shot = dice_3d(pred_one_shot, gt_zyx)
            write_mask_like(pred_one_shot, img_sitk, out_mask_path)
            print(
                f"[OK] {patient_id}: selector_dice={dice_selector:.4f}, one_shot_dice={dice_one_shot:.4f}, "
            ...
            rows.append(
                {
                    ...
                    "Middle_ID": middle_id,
                    "Dice3D_Selector": dice_selector,
                    "Dice3D_OneShot": dice_one_shot,
                }
            )
        finally:
    ...
    ws = wb.active
    ws.title = "Results"
    ws.append(["Patient_ID", "Prompt_Slice_ID", "Lower_Bound_ID", "Upper_Bound_ID", "Middle_ID", "Dice3D_Selector", "Dice3D_OneShot"])
    for r in rows:
        ws.append(
            [
                r["Patient_ID"],
                ...
                int(r["Middle_ID"]),
                round(float(r["Dice3D_Selector"]), 6),
                round(float(r["Dice3D_OneShot"]), 6),
            ]
        )
