...
# -*- coding: utf-8 -*-
"""
SAM2 finetuning with iterative two-stage prompting (online bad_95 middle):
  pass-1: upper + lower prompts with current model
  select worst middle slice online from pass-1 prediction (2D HD95)
...
def select_worst_hd95_middle_from_outputs(outputs, batch_masks: torch.Tensor, unique_objects_identifier):
    ...
    for o in range(o_dim):
        ...
        if len(valid_scores) == 0:
            ...
            runtime_map[video_id] = int(max(valid_scores, key=lambda x: x[1])[0])
    return runtime_map
...
def _runtime_middle_map_from_precomputed(unique_objects_identifier, precomputed_middle_map):
    o_dim = int(unique_objects_identifier.shape[1])
    runtime_middle_map = {}
    missing_video_ids = []
    for o in range(o_dim):
        video_id = int(unique_objects_identifier[0, o, 0].item())
        middle_id = precomputed_middle_map.get(video_id)
        if middle_id is None:
            missing_video_ids.append(video_id)
            continue
        runtime_middle_map[video_id] = int(middle_id)

    if len(missing_video_ids) > 0:
        missing_str = ", ".join(str(x) for x in sorted(set(missing_video_ids)))
        raise KeyError(
            f"Missing precomputed middle prompt for video_id(s): {missing_str}. "
            "Please regenerate precomputed middle map."
        )
    return runtime_middle_map


@torch.no_grad()
def precompute_middle_map_for_loader(
    selector_model,
    loader,
    device,
    amp_dtype,
    ...
    selector_model.eval()
    selector_core = unwrap_model(selector_model)
    precomputed_middle_map = {}
    ...
    for batch in loader:
        ...
        with torch.cuda.amp.autocast(
            ...
            selector_core.clear_runtime_middle_prompt_map()
            selector_backbone_out = _precompute_backbone_out(
                selector_core,
                batch,
                forward_backbone_per_frame=forward_backbone_per_frame,
            )
            selector_stage1 = selector_core.prepare_prompt_inputs(
                _clone_backbone_out(selector_backbone_out), batch
            )
            selector_output_dict = _forward_tracking_iterative(
                selector_core, selector_stage1, batch
            )
            selector_outputs = _outputs_from_tracking_dict(
                selector_output_dict, selector_stage1["num_frames"]
            )
        ...
        overlap = set(precomputed_middle_map.keys()).intersection(set(runtime_middle_map.keys()))
        if len(overlap) > 0:
            # Deterministic dataset has one fixed middle per video_id; keep strict to avoid silent drift.
            for vid in sorted(overlap):
                if int(precomputed_middle_map[vid]) != int(runtime_middle_map[vid]):
                    raise RuntimeError(
                        f"Conflicting middle_id for video_id={vid}: "
                        f"{precomputed_middle_map[vid]} vs {runtime_middle_map[vid]}"
                    )

        precomputed_middle_map.update({int(k): int(v) for k, v in runtime_middle_map.items()})

        selector_core.clear_runtime_middle_prompt_map()
        del selector_outputs, selector_output_dict, selector_stage1, selector_backbone_out
        del runtime_middle_map

    return precomputed_middle_map
...
def run_epoch_two_pass(
    ...
    for batch_idx, batch in enumerate(loader, start=1):
        ...
        runtime_middle_map = _runtime_middle_map_from_precomputed(
            unique_objects_identifier=batch.metadata.unique_objects_identifier,
            precomputed_middle_map=precomputed_middle_map,
        )

        # Main one-epoch training: reset to main model (initialized from initial ckpt),
        # and train with upper/lower/middle prompts in one forward.
        ...
        with torch.cuda.amp.autocast(
            ...
            main_backbone = core_model.prepare_prompt_inputs(
                _clone_backbone_out(base_backbone_out), batch
            )
            main_output_dict = _forward_tracking_iterative(
                core_model, main_backbone, batch
            )
            outputs_stage2 = _outputs_from_tracking_dict(
                main_output_dict, main_backbone["num_frames"]
            )
        ...
        del outputs_stage2, main_output_dict, main_backbone, base_backbone_out
        del runtime_middle_map, loss
        if (
            empty_cache_every > 0
...
def main():
    parser = argparse.ArgumentParser(
        "SAM2 one-epoch: selector(1st-round weights) chooses bad_95 middle; main model trains with initial weights + 3 prompts"
    )
    ...
    parser.add_argument(
        ...
        help="External TrainResults root of first-round model; used only to select worst middle slice.",
    )
    parser.add_argument(
        "--pretrained-ckpt",
        type=Path,
        default=DEFAULT_PRETRAINED_CKPT,
        help="Initial checkpoint used to start one-epoch three-prompt training (not first-round weights).",
    )
    ...
    if args.train_root is None:
        raise ValueError("Missing --train-root. Please run via run_train.py or pass it explicitly.")
    if args.output_root is None:
        raise ValueError("Missing --output-root. Please run via run_train.py or pass it explicitly.")
    if args.selector_train_output_root is None:
        raise ValueError("Missing --selector-train-output-root. Please run via run_train.py or pass it explicitly.")
    ...
    selector_ckpt = resolve_best_ckpt_from_train_output(args.selector_train_output_root)
    if is_main_process():
        print(f"[INFO] selector checkpoint resolved from {args.selector_train_output_root}: {selector_ckpt}")
        print(f"[INFO] main model initial checkpoint: {args.pretrained_ckpt}")
    ...
    for fold_idx, (train_patients, val_patients) in enumerate(fold_defs):
        ...
        if is_main_process():
            print(f"[Fold {fold_idx}] Precomputing fixed middle prompts for train split...")
        train_middle_map = precompute_middle_map_for_loader(
        ...
        if is_main_process():
            print(f"[Fold {fold_idx}] Precomputing fixed middle prompts for val split...")
        val_middle_map = precompute_middle_map_for_loader(
            selector_model=selector_model,
            loader=val_loader,
            device=device,
            amp_dtype=amp_dtype,
        ...
        if is_main_process():
            print(
                f"[Fold {fold_idx}] Middle precompute done: "
                f"train={len(train_middle_map)} videos, val={len(val_middle_map)} videos"
            )
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            tr_loss, tr_dice = run_epoch_two_pass(
                ...
                amp_dtype=amp_dtype,
                train_mode=True,
                precomputed_middle_map=train_middle_map,
                forward_backbone_per_frame=args.forward_backbone_per_frame,
            ...
            with torch.no_grad():
                va_loss, va_dice = run_epoch_two_pass(
                    ...
                    amp_dtype=amp_dtype,
                    train_mode=False,
                    precomputed_middle_map=val_middle_map,
                    forward_backbone_per_frame=args.forward_backbone_per_frame,
                    empty_cache_every=args.empty_cache_every,
                )
