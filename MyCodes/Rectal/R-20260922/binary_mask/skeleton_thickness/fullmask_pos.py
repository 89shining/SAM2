...
Tracking:
    - training uses one randomly sampled propagation direction per patient
      update (forward or backward, approximately 50/50)
    - validation/test average forward and backward probabilities before evaluation
...
DEFAULT_POS_PROMPT_NAME = "pos_skeleton.nii.gz"
...
def resize_prompt_preserve_foreground(
    ...
    """Downsample prompts without dropping tiny skeleton prompts/slices."""
...
class PositiveCorrectionDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> CorrectionBatch:
        ...
        cached = self._case_cache.get(patient_dir)
        if cached is None:
            ...
            arrays["pos_skeleton"] = arrays["pos_prompt"] > 0
            self._case_cache[patient_dir] = (reference, arrays)
        else:
        ...
        # Skeleton segment and physical thickening are intentionally never
        # clipped back to raw error, GT, nnU-Net, or a component boundary.
        pos_prompt = build_skeleton_prompt(
            arrays["pos_skeleton"], reference.GetSpacing(),
            self.thickness_mm, segment_keep, self.segment_mode,
        )
        ...
        pos_raw_tensor = resize_prompt_preserve_foreground(
            pos_raw_tensor, resize_to
        )
        prompt_tensor = resize_prompt_preserve_foreground(
            prompt_tensor, resize_to
        )
        ...
        target_tensor = torch.logical_or(
            nnunet_tensor > 0, pos_raw_tensor > 0
        ).float()
...
def stochastic_round_count(expected: float, total: int) -> int:
    """Unbiased patient-level rounding with at least one retained prompt."""
...
def patient_training_windows(
    ...
    keep_max: float,
    window_size: int,
) -> tuple[list[CorrectionBatch], dict[str, float]]:
    """Patient-level dropout, non-overlapping prompt clusters, then windows."""
    available = torch.nonzero(
        batch.positive_prompts.flatten(1).any(dim=1), as_tuple=False
    ).flatten().tolist()
    ...
    for cluster in clusters:
        ...
        stop = start + length
        prompts = torch.zeros_like(batch.positive_prompts[start:stop])
        for global_frame in cluster:
            prompts[global_frame - start] = batch.positive_prompts[global_frame]
        windows.append(
            CorrectionBatch(
                ...
                missed_masks=batch.missed_masks[start:stop],
                positive_prompts=prompts,
                case_dir=batch.case_dir,
...
def bidirectional_logits(
    ...
    prompt_frames = positive_prompt_frames(
        batch,
    ...
    if direction_mode in ("forward", "backward"):
        outputs = track_direction(
            ...
            reverse=direction_mode == "backward",
        )
        logits = torch.stack(
            [output["pred_masks_high_res"][:, 0] for output in outputs], dim=0
        )
        if return_debug:
            raise ValueError("single-direction training does not support debug output")
        return logits
...
def prompt_slices_only_logits(
    ...
    prompts = positive_prompt_frames(batch, keep_fraction=prompt_keep_fraction)
    true_prompts = set(
        int(frame)
    ...
    for frame in prompts:
        ...
        output = model.track_step(
            ...
            feat_sizes=sizes,
            point_inputs=None,
            mask_inputs=batch.positive_prompts[frame].unsqueeze(1),
...
def train_one_epoch(
    ...
    debug_batches: int,
    debug_path: Path,
    prompt_keep_min: float,
    prompt_keep_max: float,
    temporal_window: int,
) -> dict[str, float]:
    model.train(True)
    ...
    for batch_index, batch in enumerate(loader):
        ...
        if temporal_window > 0 and batch.num_frames > temporal_window:
            ...
            batch = CorrectionBatch(
                ...
                missed_masks=batch.missed_masks[start:stop], positive_prompts=batch.positive_prompts[start:stop],
                case_dir=batch.case_dir, original_hw=batch.original_hw,
            )
...
def train_one_epoch_patient_level(
    model: DualStreamSAM2Train,
    loader: DataLoader,
    optimizer: torch.optim.Optimizer,
    scaler: torch.cuda.amp.GradScaler,
    ...
    """Train with patient-level prompt dropout and case-normalized window loss."""
    model.train(True)
    criterion = DiceBCELoss().to(device)
    ...
    for batch in loader:
        ...
        append_metrics(
            audit_path,
            {"epoch": epoch + 1, "patient": batch.case_dir.name, **audit},
        )
        if not windows:
            continue
        ...
        for window in windows:
            ...
            result = calculate_metrics(logits.detach(), window)
    ...
    if case_count == 0:
        raise RuntimeError("No patient had a non-empty retained positive prompt")
...
def load_training_checkpoint(
    ...
    # Latest checkpoints created by R-20260916 retain all stochastic state so
    # resumed patient sampling and forward/backward choices continue identically.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--skeleton-thickness-mm", type=float, default=0.0)
...
def main() -> None:
    ...
    if args.skeleton_thickness_mm < 0:
        raise ValueError("--skeleton-thickness-mm must be >= 0")
    ...
    dataset_kwargs = {
        ...
        "thickness_mm": args.skeleton_thickness_mm,
    }
    train_dataset = PositiveCorrectionDataset(
        train_patients, **dataset_kwargs, training_prompt=True, segment_mode="random",
    ...
    validation_dataset = PositiveCorrectionDataset(
        validation_patients,
        **dataset_kwargs, segment_mode="center", segment_keep_min=args.eval_segment_keep,
    )
    ...
    for epoch in range(start_epoch, args.max_epochs):
        # DataLoader shuffle uses a deterministic epoch seed, so it does not
        # introduce an uncheckpointed source of randomness on resume.
        if train_loader.generator is not None:
            train_loader.generator.manual_seed(args.seed + epoch)
        train_metrics = train_one_epoch_patient_level(
            model,
            train_loader,
            optimizer,
            scaler,
            ...
            epoch=epoch,
            audit_path=args.output_root / "patient_prompt_audit.tsv",
            prompt_keep_min=args.train_prompt_keep_min,
            prompt_keep_max=args.train_prompt_keep_max,
            temporal_window=args.train_temporal_window,
        )
        ...
        if epoch + 1 > args.early_stop_start_epoch and patience_counter >= args.patience:
            print(
    ...
    if args.skip_test:
        print(f"[DONE] validation-only fine-tuning; best={best_metric:.4f}")
        return
