#!/usr/bin/env python3
"""GPU scheduler for 8 independent Gaussian-sigma models.
...
SIGMAS=(1,2,3,4); BRANCHES=('pos','neg')
...
tasks=deque((t,b) for t in SIGMAS for b in BRANCHES); active={}; retries={}
while tasks or active:
    for gpu in free_gpus(set(a.reserve_gpus)|set(active), a.min_free_mib):
        if len(active)>=a.max_parallel or not tasks: break
        t,b=tasks.popleft(); out=a.run_root/'checkpoints'/b/f's{t:02d}mm'; best=out/'checkpoints'/'best.pth'
        if best.is_file(): continue
        out.mkdir(parents=True,exist_ok=True); shutil.copy2(SPLIT,out/'split.json')
        cmd=[str(a.python),str(a.code_root/f'fullmask_{b}.py'),'--data-root',str(a.data_root),'--output-root',str(out),'--test-results-dir',str(a.run_root/'unused_train_test'/b/f's{t:02d}mm'),'--init-checkpoint','/home/wusi/SAM2/checkpoints/sam2.1_hiera_small.pt','--input-size','512','--gaussian-sigma-mm',str(t),'--max-epochs','60','--patience','10','--warmup-epochs','5','--early-stop-start-epoch','5','--train-prompt-keep-min','0.30','--train-prompt-keep-max','0.70','--eval-prompt-keep','0.50','--train-temporal-window','9','--skip-test','--num-workers','4','--device','cuda']
