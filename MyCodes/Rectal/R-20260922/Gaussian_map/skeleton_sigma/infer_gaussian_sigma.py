#!/usr/bin/env python3
"""Bidirectional inference for fixed skeleton sigmaes."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sigmas-mm", type=float, nargs="+", required=True)
        ...
        for case in cases(args):
            ...
            if args.kind == "pos":
                dataset = branch.PositiveCorrectionDataset(
                    ...
                    gaussian_sigma_mm=sigma, segment_keep_min=args.segment_fraction,
            ...
            original = gaussian_map(skeleton, ref.GetSpacing(), sigma, args.segment_fraction, "center")
            ...
            np.savez_compressed(
                ...
                valid_slices=valid,
                prompt_mask=np.where(np.isin(np.arange(original.shape[0])[:, None, None], selected), original, False).astype(np.float16),
                prompt_fraction=np.float32(args.prompt_fraction),
                skeleton_sigma_mm=np.float32(sigma),
                segment_fraction=np.float32(args.segment_fraction),
            )
            print(args.kind, sigma_tag, case.name, selected.tolist(), flush=True)
