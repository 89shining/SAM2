#!/usr/bin/env python3
"""Infer one trained branch model against every physical test-disk radius."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--test-radii", type=int, nargs="+", default=tuple(range(2, 21, 2)))
...
def main() -> None:
    ...
    for radius in args.test_radii:
        ...
        output_dir = args.output_root / f"test_r{radius:02d}mm"
        ...
        for case in cases(args):
            ...
            original = sitk.GetArrayFromImage(
                sitk.ReadImage(str(case / prompt_name))
            ) > 0
            if not original.any():
            ...
            np.savez_compressed(
                ...
                test_radius_mm=np.int16(radius),
            )
            print(args.kind, radius, case.name, selected.tolist(), flush=True)
