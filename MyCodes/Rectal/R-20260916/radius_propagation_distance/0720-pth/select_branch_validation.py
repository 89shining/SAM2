#!/usr/bin/env python3
"""Select m*(r) and r* for one branch using validation data only.

This script is deliberately downstream of training: it accepts one fixed
``best.pth`` probability cache per training radius and never inspects epochs.
...
def pick(rows: list[dict], radius_key: str, gate_key: str = "gate_mm") -> dict:
    """Dice-equivalence (0.001), then HD95, then smaller named radius/gate."""
    ...
    return min(hd_best, key=lambda row: (float(row[radius_key]), float(row[gate_key])))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--train-radii", type=int, nargs="+", default=RADII)
...
def main() -> None:
    ...
    for radius in args.train_radii:
        cache_dir = args.cache_root / args.kind / f"r{radius:02d}mm" / f"test_r{radius:02d}mm"
    ...
    for radius in args.train_radii:
        for gate in GATES:
            group = [row for row in per_case if row["train_radius_mm"] == radius and row["gate_mm"] == gate_label(gate)]
            summary.append({"kind": args.kind, "train_radius_mm": radius, "gate_mm": gate_label(gate),
    ...
    profile = [pick([row for row in summary if row["train_radius_mm"] == radius], "gate_mm") for radius in args.train_radii]
    for row in profile:
    ...
    locked = pick(profile, "train_radius_mm", "selected_gate_mm")
