...
def resize_prompt_preserve_foreground(
    ...
    """Downsample prompts without dropping tiny physical disks/slices."""
...
class PositiveCorrectionDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> CorrectionBatch:
        ...
        if self.augment_prompt and pos_prompt.any():
            augmented = np.zeros_like(pos_prompt)
            for z in range(pos_prompt.shape[0]):
                frame = pos_prompt[z]
                if not frame.any(): continue
                radius = random.choice((-2, -1, 0, 1, 2))
                if radius < 0: frame = ndimage.binary_erosion(frame, iterations=-radius)
                elif radius > 0: frame = ndimage.binary_dilation(frame, iterations=radius)
                augmented[z] = frame & pos_raw[z]
            frames = np.flatnonzero(augmented.reshape(augmented.shape[0], -1).any(1))
            if len(frames)>1 and random.random()<0.35: augmented[random.choice(frames.tolist())]=False
            pos_prompt = augmented if augmented.any() else pos_prompt
        ...
        # R-20260916: preserve the complete physical disk after resize.  It is
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--prompt-augment", action="store_true")
...
def main() -> None:
    ...
    train_dataset = PositiveCorrectionDataset(train_patients, **dataset_kwargs, augment_prompt=args.prompt_augment)
