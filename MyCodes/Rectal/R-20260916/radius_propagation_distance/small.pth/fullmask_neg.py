...
class NegativeCorrectionDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> NegativeBatch:
        ...
        if self.augment_prompt and neg_prompt.any():
            augmented=np.zeros_like(neg_prompt)
            for z in range(neg_prompt.shape[0]):
                frame=neg_prompt[z]
            ...
            neg_prompt=augmented if augmented.any() else neg_prompt
...
def main() -> None:
    ...
    train_loader = make_loader(
        NegativeCorrectionDataset(train_patients, **dataset_args, augment_prompt=args.prompt_augment),
        True,
        args.num_workers,
        args.seed,
    )
