...
def main():
    parser = argparse.ArgumentParser(
        description="Run training then testing in independent mode (SAM2 two-pass)."
    )
    ...
    print("[1/2] Training (independent)", flush=True)
    run_cmd(train_cmd, env=env, cwd=str(script_dir))

    print("[2/2] Testing (independent)", flush=True)
