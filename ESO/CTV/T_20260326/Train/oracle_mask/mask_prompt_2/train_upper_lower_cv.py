...
# -*- coding: utf-8 -*-
"""
CUDA_VISIBLE_DEVICES=0,1 torchrun --nproc_per_node=2 train_upper_lower_cv.py
...
PROJECT_ROOT = Path(__file__).resolve().parents[6]  # .../SAM2
if str(CURRENT_DIR) not in sys.path:
    sys.path.insert(0, str(CURRENT_DIR))
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from sam2_train_upper_lower import SAM2TrainUpperLowerMask
...
DEFAULT_OUTPUT_ROOT = Path("/home/wusi/SAM2/SAM2data/Eso/20260326/Train/oracle_mask/mask_prompt_2/TrainResult")
...
class UpperLowerVolumeDataset(Dataset):
    ...
    - prompt bounds are computed from GT inside model (no prompt metadata needed here)
    """
    ...
    def __init__(
        ...
        for pdir in self.patient_dirs:
            ...
            # use full case as one sample (no truncation / no clip split)
    ...
    def __getitem__(self, idx):
        ...
        frames = []
        h0, w0 = img_clip.shape[1], img_clip.shape[2]
        for local_t in range(img_clip.shape[0]):
            u8 = window_to_uint8(img_clip[local_t], self.window_center, self.window_width)
            ...
            mask_tensor = torch.from_numpy(gt_clip[local_t]).float().unsqueeze(0).unsqueeze(0)
        ...
        # Keep original size in metadata for possible downstream usage.
...
def main():
    parser = argparse.ArgumentParser("5-fold SAM2 upper/lower-mask prompt finetuning")
