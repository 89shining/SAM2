...
class BatchedVideoMetaData:
    ...
    prompt_frame_idx: torch.LongTensor
...
def collate_fn(
    ...
    step_t_prompt_frame_idx = [[] for _ in range(T)]
    ...
    for video_idx, video in enumerate(batch):
        ...
        for t, frame in enumerate(video.frames):
            ...
            for obj in objects:
                ...
                step_t_prompt_frame_idx[t].append(
                    torch.tensor(video.prompt_frame_idx, dtype=torch.long)
                )
    ...
    prompt_frame_idx = torch.stack(
        [torch.stack(x, dim=0) for x in step_t_prompt_frame_idx], dim=0
    )
